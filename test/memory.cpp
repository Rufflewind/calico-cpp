...
int main() {
    ...
    array<double> B_array(500);
    auto B = B_array.to_slice();
    ...
    std::move(B_array).as_unique_ptr();   // B is invalidated
    B = B_array.to_slice();
    assert(B.size() == 0);

    B_array.resize(50);                   // B is invalidated
    B = B_array.to_slice();

    B.to_slice(20, 25).copy_from(slice_from(C_arr).to_const());
    {
        double B_cmp_arr[] = {0, 1, 2, 3, 4, 0, 0};
        auto B_cmp = slice_from(B_cmp_arr);
